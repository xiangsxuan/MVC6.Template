...
namespace Renting.Web.Templates
{
    public class ModuleModel
    {
        ...
        public ModuleModel(String model, String controller, String area)
        {
            ...
            ControllerTestsNamespace = $"Renting.Controllers.{(!String.IsNullOrWhiteSpace(area) ? $"{area}." : "")}Tests";
            ControllerNamespace = "Renting.Controllers" + (!String.IsNullOrWhiteSpace(area) ? $".{area}" : "");
            ControllerTests = $"{controller}ControllerTests";
            Controller = $"{controller}Controller";

            Area = area;

            Type type = typeof(BaseModel).Assembly.GetType("Renting.Objects." + model) ?? typeof(BaseModel);
